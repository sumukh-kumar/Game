...
def login_pressed():
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            login.pword_input(event)
        
        if constants.active:
            constants.colour = constants.rectcol_a
        else:
            constants.colour = constants.rectcol_p
        
        if constants.pactive:
            constants.pcolour = constants.prectcol_a
        else:
            constants.pcolour = constants.prectcol_p


        #username
        text_surface = constants.base_font.render(constants.user_text,True,(255,242,0))
        constants.WIN.blit(text_surface,(constants.input_rect.x + 5, constants.input_rect.y + 5))
        pygame.draw.rect(constants.WIN,constants.colour,constants.input_rect,4) 
        
        #password
        ptext_surface = constants.pbase_font.render('*' * len(constants.puser_text),True,(255,242,0))
        constants.WIN.blit(ptext_surface,(constants.pinput_rect.x + 5, constants.pinput_rect.y + 5))    
        pygame.draw.rect(constants.WIN,constants.pcolour,constants.pinput_rect,4) 
